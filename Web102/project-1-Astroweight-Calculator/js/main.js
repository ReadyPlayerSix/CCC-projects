...
function updateDropdownOptions() {
    ...
    if (Array.from(planetSelect.options).some(option => option.value === currentValue)) {
...
function handleNaturalSortChange(event) {
    ...
    commandLog(`SORT ORDER ${state.naturalSort ? 'NATURAL' : 'ALPHABETICAL'} ACTIVATED`, 'processing');
    updateDropdownOptions();
}
